...
func BenchmarkLoop(m dsl.Matcher) {
	...
	// variable to drop, so b.Loop() is a direct substitution. The Suggest
	// references the matched group as $body (no star); the star form $*body is a
	// match-pattern spelling and is not interpolated in a replacement.
	m.Match(
		`for range $b.N { $*body }`,
	).
		Where(m["b"].Type.Is("*testing.B")).
